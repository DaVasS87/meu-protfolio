...
init()
with st.sidebar:
    v = st.number_input("Valor (€):", value=10.0)
    a_l = st.multiselect("Ativos:", ["TTE.PA", "ALV.DE", "ZURN.SW", "CVX"])
    if st.button("Comprar"):
        df = pd.read_csv(DATA_FILE)
        for a in a_l:
            p = yf.Ticker(a).history(period="1d")['Close'].iloc[-1]
            nova = pd.DataFrame({'Data': [datetime.now().strftime("%Y-%m-%d")], 'Tipo': ['Compra'], 'Ativo': [a], 'Qtd': [v/len(a_l)/p], 'Preco_Compra': [p]})
